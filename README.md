...
SF32LB52 开发板上。设备侧集成跌倒检测、手动 SOS、语音提示、表盘 UI、
...
- 语音辅助：已实现麦克风采集和降噪；关键词识别及语音求助触发功能开发中。
  当前验证配置关闭 MIC 采集。
...
- 音频链路已实现 ADC 麦克风采集和降噪，关键词识别尚未实现，语音求助触发功能开发中。
  当前验证配置启用 SOS WAV 扬声器提示并关闭 ADC 麦克风采集；两者可独立配置。恢复
  麦克风时，应用会在播放 WAV 前暂停采集、播放完成后再恢复，不能让采集与播放各自持有
  消息队列和 DMA 后并发运行。
