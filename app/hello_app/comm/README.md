...
SOS 的本地告警页面与 BLE `CALL_REQUEST` 上报不依赖麦克风采集。音频链路已实现 ADC
麦克风采集和降噪，但关键词识别尚未实现，语音触发功能开发中。当前配置保留 WAV 扬声器
提示并关闭 ADC 麦克风采集；这不影响服务发现、状态心跳、校时或紧急事件上报。恢复采集
时，应用会在播放 WAV 前暂停采集、收到播放完成消息后再恢复，避免两个流同时争用
SF32LB52 的音频消息队列和 DMA。
...
- 语音 SOS：协议预留；关键词识别尚未实现，当前不会由语音触发。
